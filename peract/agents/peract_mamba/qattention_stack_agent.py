...
from peract.agents.peract_mamba.qattention_peract_bc_agent import (
...
class QAttentionStackAgent(Agent):
    def __init__(
        ...
        mamba_agent: Union[MultiLevelMamba, SimpleModel],
    ...
    def act(self, step: int, observation: dict, deterministic=False) -> ActResult:
        ...
        _cond, _continuous_data_dict = self._prepare_mamba_data_dict(
            observation,
            trans,
            quat,
        )
        mamba_output = self._mamba_agent.conditional_sample(
            cond=_cond, mamba_gripper=self._mamba_gripper, **_continuous_data_dict
        )
        traj = mamba_output["multi"]["traj"]
        joint_positions = mamba_output["multi"]["joint_positions"].cpu().detach().numpy()
    ...
    def load_weights(self, savedir: str, backbone: str = "unet"):
        ...
        load_model_path = self._mamba_model_path
        try:
            ...
            self._mamba_agent.load_state_dict(state_dict, strict=False)
            print("mamba model loaded")
        except Exception as e:
            print("mamba model load failed")
            print(str(e))
