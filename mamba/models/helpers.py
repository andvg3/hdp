...
class WeightedChamfer(WeightedStateLoss):
    def __init__(self, weights, converage_weight=1.0, norm="l1"):
        super().__init__(weights)
        self._coverage_weight = converage_weight
        self._norm = norm

    def _loss(self, pred, targ):
        dist = pred.unsqueeze(1) - targ.unsqueeze(2)
        if self._norm == "l2":
            dist = dist**2
        elif self._norm == "l1":
            dist = torch.abs(dist)
        ...
        consistency = dist.min(1)[0]
        converage = dist.min(2)[0]

        return consistency + self._coverage_weight * converage
